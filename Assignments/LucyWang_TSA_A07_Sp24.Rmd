...
> d = 0. No differencing is needed since the series is proved to be stationary due to the ADF test result where p-value is less than 0.05. 

> q = 1. The ACF plot shows a gradual decay with certain spikes. Therefore, the model can be ARMA(1,1).

> The overall ARIMA model should be ARIMA(1,0,1). 
...
```{r}
arima_model <- Arima(deseasonal_ts, order=c(1,0,1), include.drift=TRUE)
print(arima_model[1])
```
...
> D = 1. The strong trend in Mann Kendall test suggests that seasonal differencing may still be necessary. 
...
print(arima_autofit)
```
> The order of the best ARIMA model is ARIMA(1,1,1). d = 1 in this best model, which is different that what I specified in Q4 where d = 0 with ARIMA(1,0,1). 
